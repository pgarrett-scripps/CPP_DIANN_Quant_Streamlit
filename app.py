...
from util import cached_find_peptide_indexes, cached_find_site_indexes, ratio_inf, safe_log2, calculate_accessibility
...
with st.sidebar:
    ...
    min_ms1_area = c1.number_input(
        "Min MS1 area", value=10_000, min_value=0, help='Minimum MS1 area')

    min_evidence = c2.number_input("Min Evidence", value=0.0, min_value=0.0, max_value=1.0,
                                   help='Minimum Evidence')
    min_channel_evidence = c1.number_input("Min Channel Evidence", value=0.0, min_value=0.0, max_value=1.0,
                                           help='Minimum Channel Evidence')
    max_channel_q_value = c2.number_input("Max Channel Q-value", value=0.10, min_value=0.0, max_value=1.0,
    ...
    filter_unique_sites = st.checkbox(
        "Filter for unique sites", value=False, help='Filter for unique sites')
    ...
    if st.checkbox("Replace Missing Channel with Value"):
        replace_missing_channel = st.number_input(
            "Replace Missing Channel with Value", value=10)

    run_btn = st.button('Run', use_container_width=True, type='primary',
                        disabled=fasta_file is None or len(results_file) == 0)
...
ratio_df = df.pivot_table(
    ...
    values=['Ms1.Area', 'Evidence', 'Mass.Evidence', 'Channel.Evidence', 'Q.Value', 'Global.Q.Value',
            'Channel.Q.Value'],
    aggfunc=lambda x: x if len(x) == 1 else (_ for _ in ()).throw(
        ValueError(f"Duplicate entries found for {x.index}"))
).reset_index()
...
ratio_df = ratio_df[(ratio_df['K_count'] <= max_lysine_count)
                    & (ratio_df['K_count'] >= min_lysine_count)]
...
ratio_df['Light/Heavy.Ratio'] = ratio_df.apply(ratio_inf, axis=1)
...
ratio_df['Light/Heavy.Log2Ratio'] = ratio_df['Light/Heavy.Log2Ratio'].replace([
                                                                              np.inf], fill_inf_with)
ratio_df['Light/Heavy.Log2Ratio'] = ratio_df['Light/Heavy.Log2Ratio'].replace(
    [-np.inf], -fill_inf_with)

# check if Evidence.L or Evidence.H is greater than min_evidence
ratio_df = ratio_df[(ratio_df['Evidence.L'] >= min_evidence)
                    | (ratio_df['Evidence.H'] >= min_evidence)]
...
ratio_df = ratio_df[(ratio_df['Q.Value.L'] <= max_channel_q_value) | (
    ratio_df['Q.Value.H'] <= max_channel_q_value)]

# check if Ms1.Area.L or H is greater than min_ms1_area
ratio_df = ratio_df[(ratio_df['Ms1.Area.L'] >= min_ms1_area)
                    | (ratio_df['Ms1.Area.H'] >= min_ms1_area)]

fasta_df = to_df(fasta_file)

# check if any result proteins dont match to the fasta file
protein_ids = set()
for protein_id in ratio_df['Protein.Ids'].str.split(';'):
    if protein_id is not None:
        protein_ids.update(protein_id)

missing_proteins = protein_ids - set(fasta_df['unique_identifier'])

if len(missing_proteins) > 0:
    st.error(
        f"The following proteins from the results file were not found in the FASTA file: {', '.join(missing_proteins)}")
    st.stop()
...
with st.expander("Warnings", expanded=True):
    ...
    for index, row in ratio_df.iterrows():
        ...
        for protein_name in protein_names:
            ...
            peptide_indexes = cached_find_peptide_indexes(
                protein_sequence, stripped_sequence)
            ...
            for peptide_index in peptide_indexes:
                ...
                site_indexes = cached_find_site_indexes(
                    stripped_sequence, 'K')
        ...
        peptide_strings.append(
            ';'.join([';'.join(x) for x in peptide_strs]))
        protein_strings.append(
            ';'.join([';'.join(x) for x in protein_strs]))
    ...
    if should_filter_sites:
        ratio_df = ratio_df[ratio_df['Protein.Site.Strings'].isin(
            filter_sites)]
...
site_stats_df['Accessibility'] = 100 * 2 ** site_stats_df['Log2Ratio.Mean'] / \
    (2 ** site_stats_df['Log2Ratio.Mean'] + 1)
...
site_stats_df = site_stats_df[~site_stats_df['Protein.Site.Strings'].isin([
                                                                          None, np.nan, ""])]
